...
from ai_engine_old import ai_engine
...
async def get_user_recommendations(user_id: str, top_k: int = 10):
    try:
        ...
        # First, get admin-assigned courses (not yet started or in progress)
        try:
            assigned_progress = await db.get_user_progress(user_id)
            for progress in assigned_progress:
                if progress.get('status') in ['not_started', 'in_progress']:
                    ...
                    if course_id and course_id not in seen_course_ids:
                        ...
                        if course:
                            recommendations.append({
                                'course': course,
                                'confidence_score': 0.95,  # High confidence for admin assignments
                                'source': 'assigned'
                            })
                            seen_course_ids.add(course_id)
        except Exception as e:
            logger.warning(f"Error fetching assigned courses: {e}")
        
        # Then, get AI recommendations based on learning goals
        try:
            preferences = await db.get_user_preferences(user_id)
            
            if preferences and preferences.get('learning_goals'):
                ...
                # Add AI recommendations that aren't already assigned
                for result in results:
            ...
            logger.warning(f"Error fetching AI recommendations: {e}")
...
@app.post("/api/admin/assign-user-course")
async def assign_user_to_course(request: Request, data: dict):
    ...
    try:
        ...
        result = await db.admin_assign_user_to_course(
            session_token=session_token,
            user_id=user_id,
            course_id=course_id
        )
        ...
        if not result.get('success'):
            raise HTTPException(status_code=400, detail=result.get('error', 'Failed to assign user to course'))

        return result
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error assigning user to course: {e}")
        raise HTTPException(status_code=500, detail=str(e))
